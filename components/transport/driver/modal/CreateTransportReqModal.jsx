...
import styles from "@/components/transport/driver/modal/AddReqTransportButton.module.css";
import Select2SearchCity from "@/components/transport/driver/modal/Select2SearchCity";
import SuccessMessage from "@/components/utils/Success/SuccessMessage"; // Import SuccessMessage
...
const CreateTransportReqModal = ({ onClose, onRequestAdded, transport }) => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const url = Config.getApiUrl("transport", "all_locations");
  ...
  const calculateRoute = async (origin_id, destination_id, capacity) => {
    try {
      const url = Config.getApiUrl("transport", "calculate_route/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = Config.getApiUrl("transport", "create_transport_req");
  ...
  return (
    <>
      ...
      >
        <h2>
          درخواست حمل و نقل {transport ? `برای ${transport.car_name}` : ""}
        </h2>
              ...
              <div className={styles.badgeContainer}>
                <div className={styles.badge}>
                  <span>
                    ...
                    که حدود <strong>{estimatedDistance}</strong> کیلومتر
                    می‌باشد.
                  </span>
                  <span>
                    محدوده قیمتی از
